...
def end_time():
    ...
    cursor.execute("update study_timer.study_timer_table set end_time = current_time where entry_no = %s", (last_entry_no,))
    cursor.execute("select * from study_timer.study_timer_table where entry_no = %s", (last_entry_no,))
    record = cursor.fetchall()
    record = record[0]
    start_time = record[2]
    ...
    print(time_spent)
    cursor.execute("update study_timer.study_timer_table set time_spent = %s where entry_no = %s", (time_spent,last_entry_no))
